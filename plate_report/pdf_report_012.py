...
def create_pdf_doc(plate, main_tissue):
    pdf_filename = f"{plate}/{plate}_report_{main_tissue}.pdf"
...
def build_pdf_report(
    ...
    if this_plate in ['igvf_007','igvf_008','igvf_008b']:
        add_table(elements, cb_results, title= f"Table {table_num}: Cellbender stats (calculated for all {this_sampletype} in subpools, all tissues)")
    else:
        add_table(elements, cb_results, title= f"Table {table_num}: Cellbender stats (calculated for all {this_sampletype} in subpools, both tissues)")
    ...
    ### SECTION 4: Barcode sample map -- for main tissue only
