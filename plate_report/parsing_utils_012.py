...
def summarize_cell_counts(qc_200, qc_500, sampletype):
    n_cells_col = f'Number of {sampletype}'
    cells_200 = sum(qc_200[n_cells_col])
    cells_500 = sum(qc_500[n_cells_col])
    return (f"{cells_200:,}", f"{cells_500:,}")

def load_alignment_stats(plate, subpools):
    reports = []
...
# def load_qc_stats(plate, subpools, sampletype):
#     def _load_and_rename(file_path, umi_thresh):
#         df = pd.read_csv(file_path)
#         df['Subpool'] = subpool
#         subset = df[[
#             f'Subpool',
...
#     for subpool in subpools:
#         file_path = f"/dfs9/seyedam-lab/erebboah/parse_pipeline/pipeline_report/{plate}/{subpool}/report.csv"
#         qc_200_list.append(_load_and_rename(file_path, 200))
#         qc_500_list.append(_load_and_rename(file_path, 500))
#     return pd.concat(qc_200_list, ignore_index=True), pd.concat(qc_500_list, ignore_index=True)


def load_qc_stats(obs, main_tissue, subpools, sampletype):
    qc_200_list = []
    qc_500_list = []

    for subpool in subpools:
        obs_subset = obs[(obs['Tissue'] == main_tissue) & (obs['subpool'] == subpool)]

        def _get_qc_subset(obs_subset, umi_thresh):
            obs_thresh = obs_subset[obs_subset['total_counts_raw'] >= umi_thresh]
            return pd.DataFrame({
                'Subpool': [subpool],
                f'Number of {sampletype}': [obs_thresh.shape[0]],
                'Median UMIs': [int(obs_thresh['total_counts_raw'].median())],
                'Mean UMIs': [round(obs_thresh['total_counts_raw'].mean(), 1)],
                'Median genes': [int(obs_thresh['n_genes_by_counts_raw'].median())],
                'Mean genes': [round(obs_thresh['n_genes_by_counts_raw'].mean(), 1)]
            })

        qc_200_list.append(_get_qc_subset(obs_subset, 200))
        qc_500_list.append(_get_qc_subset(obs_subset, 500))

    # Combine all the individual subpool results into one DataFrame
    qc_200 = pd.concat(qc_200_list, ignore_index=True)
    qc_500 = pd.concat(qc_500_list, ignore_index=True)
...
def load_cellbender_stats(plate, subpools, sampletype):
    all_cb_results = []
    for subpool in subpools:
        ...
        all_cb_results.append(cb_result)

    final_cb = pd.concat(all_cb_results, ignore_index=True)
    final_cb = final_cb[['Subpool', 'Percent counts removed', 'Found cells', 'Mean denoised counts']]
    final_cb = final_cb.rename(columns={'Found cells': f'Found {sampletype}'})
    return final_cb

def load_cellbender_settings(plate, subpools):
    dfs = []
    for subpool in subpools:
    ...
    cb_settings = pd.concat(dfs, ignore_index=True)
    cb_settings = cb_settings.rename(columns={'subpool': 'Subpool'})
    cb_settings['Subpool'] = pd.Categorical(cb_settings['Subpool'], categories=subpools, ordered=True)
    return cb_settings.sort_values('Subpool')
...
def create_barcode_sample_map(plate, kit, chemistry):
    ...
    acc_map.to_csv(f'{plate}/{plate}_barcode_sample_map.tsv', sep = '\t', index = False)
